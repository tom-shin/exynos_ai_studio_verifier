...
def test_model_integrity_johnny(modle_path):
    print("\n>> ONNX inference validation")
    session = onnxruntime.InferenceSession(modle_path)
    try:
        input_data = {}
        for input in session.get_inputs():
            ...
            input_data[input_name] = np.random.rand(
                *[dim if isinstance(dim, int) else 1 for dim in input_shape]).astype(np.float32)
